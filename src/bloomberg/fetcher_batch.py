...
def fetch_single_option(ticker: str, use_overrides: bool = True) -> Optional[Dict[str, Any]]:
    """
    Récupère les données pour un seul ticker (wrapper pour compatibilité).
    
    Args:
        ticker: Ticker Bloomberg
        use_overrides: Si True, utilise les overrides
        
    Returns:
        Dictionnaire avec les données ou None
    """
    results = fetch_options_batch([ticker], use_overrides)
    raw_data = results.get(ticker, {})
    
    if not raw_data or all(v is None for v in raw_data.values()):
        return None
    
    return extract_best_values(raw_data)


if __name__ == "__main__":
    """Test du fetcher batch"""
    print("=" * 70)
    print("TEST BLOOMBERG BATCH FETCHER")
    print("=" * 70)
    print()
    
    # Test 1: Un seul ticker
    print("Test 1: Un seul ticker")
    print("-" * 70)
    ticker = "ERF6C 97.5 Comdty"
    data = fetch_single_option(ticker)
    ...
    # Test 2: Batch de tickers
    print("Test 2: Batch de plusieurs tickers")
    print("-" * 70)
    tickers = [
        "ERF6C 97.5 Comdty",
        "ERF6P 97.5 Comdty",
    ...
    batch_data = fetch_options_batch(tickers)
    
    for ticker in tickers:
        data = batch_data.get(ticker, {})
        extracted = extract_best_values(data) if data else None
        
        if extracted and extracted['premium'] > 0:
            print(f"\n{ticker}:")
            print(f"  Premium: {extracted['premium']:.4f}")
            print(f"  Delta: {extracted['delta']:.4f}")
            print(f"  IV: {extracted['implied_volatility']:.2%}")
        else:
            print(f"\n{ticker}: ✗ Pas de données")
