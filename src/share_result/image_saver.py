...
def save_payoff_diagram_png(
    ...
    from app.payoff_diagram import create_payoff_diagram
...
def save_payoff_single_strategy(
    comp,
    ...
    """
    Sauvegarde le diagramme de payoff d'une seule stratégie en PNG.

    Args:
        comp: StrategyComparison unique
        mixture: Tuple (prices, probabilities, mean) optionnel
        filename: Nom du fichier PNG de sortie
        underlying_price: Prix spot optionnel
    ...
    if comp.breakeven_points:
        be_kwargs = dict(
            ...
            marker=dict(size=10, color=color, symbol="circle-open",
                        line=dict(width=2)),
            hovertemplate="<b>Breakeven</b><br>%{x:.4f}<extra></extra>",
        )
        if mixture is not None:
            fig.add_trace(go.Scatter(**be_kwargs), secondary_y=False)
        else:
            fig.add_trace(go.Scatter(**be_kwargs))

    # Zero line
    fig.add_hline(y=0, line_dash="dash", line_color="gray", opacity=0.5,
                  **({"secondary_y": False} if mixture is not None else {}))
    ...
    # Gaussian mixture overlay
    if mixture is not None:
        prices_mix, probs_mix, _ = mixture
        fig.add_trace(
            go.Scatter(
                x=prices_mix, y=probs_mix,
    ...
    return save_figure_to_png(fig, filename, width=900, height=400)
