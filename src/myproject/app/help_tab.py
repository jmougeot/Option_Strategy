...
def display_help_tab():
    ...
    **Sommaire:**
    ...
    st.subheader("🔬 Critères Avancés (Greeks)", anchor="criteres-avances")
    
    # Delta Neutral
    with st.expander("Δ Delta Neutral"):
        st.markdown("""
        **Définition:** Mesure à quel point la stratégie est neutre par rapport aux 
        mouvements directionnels du sous-jacent.
        
        **Formule:**
        $$\\Delta_{total} = \\sum_{i} sign_i \\times \\Delta_i$$
        
        **Interprétation:**
        - **|Δ| ≈ 0** : Position neutre (ni haussière ni baissière)
        - **Δ > 0** : Position haussière (profite si le marché monte)
        - **Δ < 0** : Position baissière (profite si le marché baisse)
        - **|Δ| plus faible = Meilleur** (pour neutralité)
        
        **Usage:** Activez ce critère si vous voulez des stratégies non-directionnelles.
        """)
    
    # Gamma Low
    with st.expander("Γ Gamma Low"):
        st.markdown("""
        **Définition:** Le gamma mesure la sensibilité du delta aux mouvements du sous-jacent.
        Un gamma faible signifie que la position reste stable.
        
        **Interprétation:**
        - **Γ élevé** : Le delta change rapidement, nécessite des ajustements fréquents
        - **Γ faible** : Position stable, peu de rééquilibrage nécessaire
        - **|Γ| plus faible = Meilleur**
        
        **Note:** Les positions vendeuses ont généralement un gamma négatif.
        """)
    
    # Vega Low
    with st.expander("ν Vega Low"):
        st.markdown("""
        **Définition:** Sensibilité du prix de la stratégie aux changements de volatilité implicite.
        
        **Interprétation:**
        - **Vega > 0** : Profite si la volatilité augmente
        - **Vega < 0** : Profite si la volatilité diminue
        - **|Vega| faible** : Moins sensible aux changements de volatilité
        - **|ν| plus faible = Meilleur** (pour réduire le risque de volatilité)
        """)
    
    # Theta Positive
    with st.expander("Θ Theta Positive"):
        st.markdown("""
        **Définition:** Le theta mesure la perte de valeur temps par jour.
        
        **Interprétation:**
        - **Θ > 0** : La stratégie gagne de l'argent avec le passage du temps (vendeur net)
        - **Θ < 0** : La stratégie perd de l'argent avec le temps (acheteur net)
        - **Θ plus élevé = Meilleur**
        
        **Exemple:** Un short straddle a un theta positif élevé.
        """)
    
    # Standard Deviation (Sigma P&L)
    with st.expander("σ Standard Deviation (Écart-type du P&L)"):
        st.markdown("""
        **Définition:** La volatilité du P&L de la stratégie selon la distribution de probabilité.
        
        **Formule:**
        $$\\sigma_{P\\&L} = \\sqrt{\\int (P\\&L(S) - PM)^2 \\cdot f(S) \\, dS}$$
        
        **Interprétation:**
        - **σ faible** : P&L prévisible, faible incertitude
        - **σ élevé** : P&L très variable, forte incertitude
        - **Plus faible = Meilleur** (pour réduire le risque)
        
        **Ratio de Sharpe simplifié:** PM / σ
        """)
