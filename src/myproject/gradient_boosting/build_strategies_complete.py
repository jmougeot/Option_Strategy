...
def normalize_and_export_mapping(trade_csv_path: str , output_mapping_path : Optional[str]= None ):
    """
    ...
    for idx, row in df.iterrows():
        ...
        # Détecter le type
        strategy_type, option_type = detect_strategy_type_simple(strategy_str, len(strikes))
        result['strategy_type'] = strategy_type
        result['option_type'] = option_type
        
        # Normaliser si possible
        if strikes and strategy_type != 'unknown':
            normalized, underlying, month_expiry, year_expiry = normalize_simple(strategy_str, strikes, option_type, strategy_type)
            if normalized:
...
def detect_strategy_type_simple(strategy_str: str, num_strikes: int) -> Tuple[str, str]:
    ...
    if 'fly' in strategy_lower:
        if 'broken' in strategy_lower or 'brk' in strategy_lower or 'bkn' in strategy_lower:
    ...
    elif 'spread' in strategy_lower or ' cs' in strategy_lower or ' ps' in strategy_lower:
...
def normalize_simple(strategy_str: str, strikes: List[float], option_type: str, strategy_type: str) -> Tuple[str, str, str, str]:
