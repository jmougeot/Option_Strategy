...
from myproject.gradient_boosting.data_bulilder import (
    data_frame_bloomberg,
    prepare_dataframe_for_xgboost,
)
...
def xgboost_pretrain_and_finetune(
    ...
    model.fit(X_bloomberg, y_bloomberg, eval_set=[(X_tm_test, y_tm_test)], verbose=50)
    ...
    print(
        f"   Bloomberg Train - RMSE: {pretrain_metrics['train_rmse']:.4f}, MAE: {pretrain_metrics['train_mae']:.4f}, R²: {pretrain_metrics['train_r2']:.4f}"
    )
    print(
        f"   Trade Monitor Test - RMSE: {pretrain_metrics['test_rmse']:.4f}, MAE: {pretrain_metrics['test_mae']:.4f}, R²: {pretrain_metrics['test_r2']:.4f}"
    )
    ...
    print(
        f"   Trade Monitor Train - RMSE: {finetune_metrics['train_rmse']:.4f}, MAE: {finetune_metrics['train_mae']:.4f}, R²: {finetune_metrics['train_r2']:.4f}"
    )
    print(
        f"   Trade Monitor Test - RMSE: {finetune_metrics['test_rmse']:.4f}, MAE: {finetune_metrics['test_mae']:.4f}, R²: {finetune_metrics['test_r2']:.4f}"
    )
    ...
    print(
        f"   RMSE: {improvement_rmse:+.4f} ({improvement_rmse/pretrain_metrics['test_rmse']*100:+.2f}%)"
    )
    print(f"   R²: {improvement_r2:+.4f}")

    # Feature importance
    feature_importance = pd.DataFrame(
        {"feature": X_bloomberg.columns, "importance": model.feature_importances_}
...
def predict_and_rank_strategies(
    model, strategies: List[StrategyComparison], top_n: int = 10
