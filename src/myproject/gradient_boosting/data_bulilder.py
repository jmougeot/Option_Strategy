...
def calculate_strategy_score(strategy: StrategyComparison) -> float:
    ...
    if (
        "fly" in strategy_name_lower
        or "butterfly" in strategy_name_lower
        or "condor" in strategy_name_lower
    ):
        score += 5
...
def data_frame_bloomberg(
    ...
    for s in strategies:
        ...
        feats.append(
            s.strategy_name if hasattr(s, "strategy_name") else None
        )  # original
        feats.append(getattr(s, "underlying", None))  # underlying
        feats.append(
            s.expiration_month if hasattr(s, "expiration_month") else None
        )  # month_expiracy
        feats.append(
            s.expiration_year if hasattr(s, "expiration_year") else None
        )  # year_expiray
        ...
        feats.append(
            str([opt.strike for opt in s.all_options])
            if hasattr(s, "all_options")
            else None
        )  # strikes
        ...
        feats.append(
            s.surface_profit_ponderated if s.surface_profit_ponderated else None
        )
        feats.append(s.surface_loss if s.surface_loss else None)
        feats.append(s.surface_profit if s.surface_profit else None)
        feats.append(
            s.risk_reward_ratio_ponderated if s.risk_reward_ratio_ponderated else None
        )
        ...
        feats.append(
            getattr(s, "is_trade_monitor_data", False)
        )  # is_trade_monitor_data
