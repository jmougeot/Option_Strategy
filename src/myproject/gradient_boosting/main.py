from myproject.gradient_boosting.bloomberg_to_strat import (
    process_bloomberg_to_strategies,
)
from myproject.gradient_boosting.model import (
    xgboost_pretrain_and_finetune,
    predict_and_rank_strategies as xgb_predict,
)
from myproject.app.utils import strike_list
from myproject.app.widget import ScenarioData
from myproject.gradient_boosting.build_strategies_complete import (
    normalize_and_export_mapping,
)
...
def SFR():
    scenario: ScenarioData = ScenarioData(
        [96.03, 96.35, 96.1, 96.85, 96.35],
        [0.03, 0.03, 0.03, 0.03, 10],
        [15, 60, 15, 10, 3],
    )
    underlying = "SFR"
    step = 0.0625
...
trade_monitor_path = (
    Path(__file__).parent.parent / "trade_monitor_data" / "Trade_monitor.csv"
)
mapping_csv_path = (
    Path(__file__).parent.parent / "trade_monitor_data" / "Strategy_mapping.csv"
)
...
if not trade_monitor_path.exists():
    raise FileNotFoundError(
        f"Le fichier Trade_monitor.csv n'existe pas: {trade_monitor_path}"
    )
...
if not mapping_csv_path.exists():
    ...
    mapping_df, scores = normalize_and_export_mapping(
        str(trade_monitor_path), str(mapping_csv_path)
    )
...
model, pretrain_metrics, finetune_metrics, feature_importance = (
    xgboost_pretrain_and_finetune(
...
best_bloomberg_strategies = xgb_predict(
    model=model, strategies=bloomberg_strategies, top_n=10
)
